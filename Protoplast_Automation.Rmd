...
TODO - how to manually compensate?
TODO - this is what it looks like without compensation
...
TODO - what would it look like if the data is already transformed and do we need to do the following step?
...
To begin quantifying changes in reporter gene expression between our constructs, we first need to identify the live transformed cells based on their expression of red fluorescent protein (RFP) from the pBeaconRFP vector. TODO - dead cells should not fluoresce. TODO - in order to id this pop of cells, we need a non-transformed control sample. 
...
Next, we will backgate this population of RFP positive viable cells to a FSC.A vs. SSC.A plot to visualize all live cells. To do this, we will plot these 'RFP_high' events on a FSC.A vs SSC.A plot (AKA backgating) to determine where viable cells fall. We can draw a new gate (TODO - that encompasses 99% blah blah) around these events and define it as `viable_cells`. TODO - this helps us exclude the majority of debris from our analysis.

TODO - explain what `gate_flowclust_2D()` does
TODO- quantile - change value to be more stringent

TODO - code for transformation rate - LATER - put after cells gate
...
The default prediction level in the `gate_singlet()` function is set to `0.99` meaning that 1% of cells are estimated to be doublets. You can change the doublet estimation by changing the value assigned to the `prediction_level` argument in the `gate_singlet()` function. TODO - explain that ours is set to .95
...
TODO Now we have identified the population of viable cells that we want to measure the reporter gene expression in response to our gene of interest.
...
TODO - we readjust our gate now that we have excluded the majority of debris with high autofluourescence and thus we can lower our RFP positive threshold. Explain how to adjust.

TODO- transformation rate - explain that it might be fairly conservative.
...
```{r include = FALSE}
# # 2) Get emmeans on log10 scale
# em <- emmeans(data.aov, "construct")
# 
# # 3) Back‑transform to get geometric means & 95% CI on original scale
# #    type = "response" + transform via inverse of log10
# em_resp <- summary(em, type = "response", infer = TRUE)
# 
# # 4) Compute fold‑change relative to GUS
# gus_mean <- em_resp %>% filter(construct == "GUS") %>% pull(emmean)
# 
# plot_df <- em_resp %>%
#   mutate(
#     Fold    = emmean / gus_mean,
#     Fold_lo = lower.CL / gus_mean,
#     Fold_hi = upper.CL / gus_mean
#   )
# 
# # suppose plot_df has construct, Fold, Fold_lo, Fold_hi
# letters_df <- data.hsd$groups %>%
#   as_tibble(rownames="construct") %>%
#   dplyr::select(construct, groups)
# 
...
# 
# # Reordering the samples for plotting
# plot_df2$construct <- factor(plot_df2$construct, levels = relevels)
# 
# # 5) Plot
# jpeg("Relative_ChangeRev.jpg", width = 1250, height = 2500, res = 500) 
# ggplot(plot_df2, aes(x = construct, y = Fold, fill = construct)) +
#   geom_col(color = "black", width = 0.6) +
#   geom_errorbar(aes(ymin = Fold_lo, ymax = Fold_hi),
#                 width = 0.1, size = 0.8) +
#   scale_fill_manual(values = c("GUS" = "#fda732",
#                                "GUS + IAA" = "#ef864d",
#                                "ARF7"     = "#f9d02f")) +
#   geom_text(aes(label=groups), vjust=-1) +
#   ylab("Fold-change vs GUS (geometric mean)") +
#   xlab(NULL) +
#   theme_classic() +
#   theme(legend.position="none",
